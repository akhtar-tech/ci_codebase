	...
	*/
	class File_upload
	{
		...
		public function __construct()
		{
			...
			$this->file_table = array(['table' => 'profile', 'column' => 'image', 'path' => './data/profile/']);
		}
		...
		public function upload(array $data)
		{
			...
			if ($data['file']) {
				...
				if ($data['remote_upload_url'] && isset($this->CI->config->config['remote_upload']) && $this->CI->config->config['remote_upload'] == TRUE) {
					$url = $data['remote_upload_url'];
					...
					/*$this->CI->load->library('upload', $config);
						if (!$this->CI->upload->do_upload($data['key'])) {
						
						$status = FALSE;
						$result = $errors = array('error' => $this->CI->upload->display_errors());
						} else {
						
						$status = TRUE;
						$result = $file_data = array('upload_data' => $this->CI->upload->data());
		...
		public function all_files($path = NULL, $depth = NULL){
			//$this->CI->load->helper('function_helper');
