<?php
	...
	*/
	class MY_Model extends CI_Model
	{
		...
		{
			parent::__construct();
			$this->get_local_config('api_config');
			if (isset($this->config->config['debug_mode'])) {
				$this->debug_mode = $this->config->config['debug_mode'];
			}
		...
		{
			$error = $this->db->error();
			if ($this->debug_mode == TRUE && $this->config->item('rest_debug') == TRUE) {
				$this->db_error = array('message' => $error['message'], 'error' => $error['code']);
				} else if ($this->config->item('rest_debug') == FALSE) {
